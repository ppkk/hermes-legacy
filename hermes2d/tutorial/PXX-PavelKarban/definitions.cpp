...
public:
    ...
    void add_magnetic_material(std::string marker, double permeability, double conductivity, double external_current_density)
    {
        ...
        {
            add_matrix_form(new WeakFormsH1::VolumetricMatrixForms::DefaultMatrixFormVol(0, 1, marker, - 2 * M_PI * frequency * conductivity, NULL, HERMES_NONSYM, HERMES_PLANAR));
            add_matrix_form(new WeakFormsH1::VolumetricMatrixForms::DefaultMatrixFormVol(1, 0, marker,   2 * M_PI * frequency * conductivity, NULL, HERMES_NONSYM, HERMES_PLANAR));
        }
        // external current density
        if (fabs(external_current_density) > EPS_ZERO)
            add_vector_form(new WeakFormsH1::VolumetricVectorForms::DefaultVectorFormVol(0, marker, external_current_density, NULL, HERMES_PLANAR));
    }
};
